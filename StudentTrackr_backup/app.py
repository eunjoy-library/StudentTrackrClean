...
try:
    # 환경 변수에서 자격 증명 확인 (유일한 인증 방식)
    firebase_json = os.environ.get("FIREBASE_CREDENTIALS_JSON")
    
    if not firebase_json:
        logging.critical("❌ 'FIREBASE_CREDENTIALS_JSON' 환경변수가 설정되지 않았습니다.")
        raise RuntimeError("Firebase 설정 실패: 환경변수를 설정해주세요.")
    
    try:
        # JSON 문자열을 직접 파싱하여 사용
        cred_dict = json.loads(firebase_json)
        cred = credentials.Certificate(cred_dict)
        logging.info("Firebase 자격 증명: 환경 변수에서 JSON으로 직접 로드")
    except json.JSONDecodeError as je:
        logging.critical(f"❌ JSON 파싱 오류: {je}")
        raise ValueError(f"❌ Firebase 환경변수가 유효한 JSON 형식이 아닙니다: {je}")
